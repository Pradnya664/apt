...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH: {
            ...
            // use the ANSI GetModuleHandleA with a narrow string literal
            HMODULE hKernel = GetModuleHandleA("kernel32.dll");
            if (hKernel != NULL) {
                VirtualAlloc_t virtualAlloc = (VirtualAlloc_t)GetProcAddress(hKernel, "VirtualAlloc");
                VirtualFree_t  virtualFree  = (VirtualFree_t) GetProcAddress(hKernel, "VirtualFree");
